...
export default class CheckRoom extends Connection {
    /**
     * @Function Login 
    ...
    public getRoomDetail = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            var rooms: any = await this.getOfDB(sql, [building_id]);
    ...
    public getQRCode = async (req: Request, res: Response, next: NextFunction) => {
        const { text_url } = process.env;

        try {
            var qrcode = await QRCode.toDataURL(text_url + "");
    ...
    public checkActivityRoom = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            var activities: any = await this.getOfDB(`select b.*,r.*,bm.address,bm.building_name,l.* from room_master r 
            ...
            var response = {
                ...
                data: activities.map((item: any) => {
                    ...
                    return_item.date = moment(item.start_date).format("DD/MM/YYYY");
                    return_item.time = moment(item.start_date).format("HH:mm") + " - " + moment(item.end_date).format("HH:mm");
    ...
    public viewDetail = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            var activities: any = await this.getOfDB(sql, [booking_id]);
            var response = {
                code: 1, message: "ok",
                data: activities.map((item: any) => {
                    var return_item: any = {};
